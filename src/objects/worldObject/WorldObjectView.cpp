...
/* Constructor */
WorldObjectView::WorldObjectView(const GLchar* path) {
    this->loadModel(path);
}

/* Functions */
void WorldObjectView::Draw(Shader shader) {
...
void WorldObjectView::loadModel(string path) {
    // Read using assimp
...
void WorldObjectView::processNode(aiNode* node, const aiScene* scene) {
...
Mesh WorldObjectView::processMesh(aiMesh* mesh, const aiScene* scene) {
    // Mesh Data
...
vector<Texture> WorldObjectView::loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName) {
    vector<Texture> textures;
...
GLint WorldObjectView::TextureFromFile(const char* path, string directory) {
