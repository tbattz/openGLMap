...
void mavlinkMain(string host, string port) {
	...
	try {
		...
		boost::asio::ip::address::from_string(host), boost::lexical_cast<int>(port));
		std::cout << "Local bind " << local_endpoint << std::endl;

		// Create Socket
		udp::socket socket(io_service, udp::endpoint(udp::v4(), boost::lexical_cast<int>(port)));
